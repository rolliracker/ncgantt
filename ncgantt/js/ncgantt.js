//"use strict";
	...
	// globals --->
		...
		let boards = null;
        const stackColors = {};
		const cardFilter = {'labels':[], 'stacks':[]};  // e.g. cardFilter.label = ["important",]
		...
		let cardId2taskIndex = {};
		let enforceRefreshAfterInteraction = false;
		
		// monitor network state
		let isOnline = window.navigator.onLine;
		let networError = false;
		
		let edit_symbol = '<img src="img/pencil2.svg">';
		let confirm_symbol = '<img src="img/check-square-outlined.svg">';
	// <---
	...
	// Nextcloud section --->
		...
		function setupEnvironment(nextcloud) {
			if (nextcloud) {  	// hide settings when in Nextcloud
				let el = document.getElementById('settingsContainer');
				el.classList.add("hidden");
				
				edit_symbol = '<img src="../../custom_apps/ncgantt/img/pencil2.svg">';
				confirm_symbol = '<img src="../../custom_apps/ncgantt/img/check-square-outlined.svg">';
				
			} else {			// show settings when outside of Nextcloud
				let el = document.getElementById('settingsContainer');
	...
	// Event listeners --->
		async function setupEventListeners() {
			...
			// Hook up the Export and Import Button
	...
	// API communication and synchronization --->
        async function makeApiCall(endpoint, method = 'GET', body = null) {
			const apiVersion = 'v1.1';
			
			try {
				//console.log("Online:", window.navigator.onLine);
				if(!window.navigator.onLine) {  // not reliable: window.navigator.onLine can return true even with WLAN turned off
				...
				if (isNextcloud) {
	 			...
	 			//console.log(baseUrl);
	/*			const has_internet = await hasInternet(baseUrl);
				if (!has_internet) {
					//return;
				}
				console.log("...checking internet done");
	 */            
				...
				if (!response.ok) {
					...
					if (!isOnline || networError) {
						showSuccess("You are online");
					}
					networError = false;
					isOnline = true;
				}
		...
		/* async function hasInternet(url) {  // Source: https://fmennen.de/post/checking-internet-connection-in-java-script
			fetch(url).then(response => {
				...
				// Connection is not active
				console.log('No internet connection xx', error);
			});
		}
		*/     
		/* const checkOnlineStatus = async () => {   // Source: https://www.freecodecamp.org/news/how-to-check-internet-connection-status-with-javascript/
		  try {
			const online = await fetch("/img/app.svg");
        ...
        async function sendCardData(boardId, stackId, cardId, card) {
            ...
            try {
                ...
                showError(error.message);
				pendingCardUpdates[cardId] = card;
                //throw error;
            }
        }
		async function sendPendingCardData() {
			if( !Object.keys(pendingCardUpdates).length ) {
				return;
			}
			//console.log("sendPendingCardData...");
			for (const [cardId, card] of Object.entries(pendingCardUpdates)) {
				//console.log(cardId, card);
				const bordId = card.boardId;
				const stackId = card.stackId;
				try {
					result = await sendCardData(bordId, stackId, cardId, card);
					//console.log(result);
					if (result) {
						//console.log("...delete pending data!!!");
						delete pendingCardUpdates[cardId];  // remove card after successful sending
					}
				} catch {
		...
		async function markPendingCardData() {
			if(popupIsOpen) {
				return;
			}
			//console.log("markPendingCardData...");
			...
			// console.log(pendingCardUpdates);
			// for (const [cardId, card] of Object.entries(pendingCardUpdates)) {
				// console.log(cardId, card);
				// const taskIndex = cardId2taskIndex[cardId];
				// console.log(cardId, card, taskIndex);
				// const tasks = ganttChart.tasks;
	...
	// Import / Export ---->
		function exportDictAsJSON(dict, filename) {
			if (!dict || dict.constructor != Object) {  // check if dict is really a dictionary
				return;
			}
		...
		async function importBoard(importedBoardData) {
			try{
				...
				const boardId = await importDeckBoard(importedBoardData, makeApiCall);
		...
		async function importDeckBoard(boardData, makeAPICall) {
			console.log("importDeckBoard...");
		  try {
			// Check if a board with the same title already exists
			const existingBoards = await makeAPICall('/boards', 'GET');
			const existingBoard = existingBoards.find(board => (board.title === boardData.title && !board.deletedAt));
			...
			if (existingBoard) {
			  ...
			  const overwrite = confirm(`A board named "${boardData.title}" already exists. Do you want to overwrite it?`);
			  ...
			  try {
				await makeAPICall(`/boards/${existingBoard.id}`, 'DELETE');
				console.log(`Deleted existing board: ${existingBoard.title}`);
			  } catch (error) {
				  console.error(`Error while deleting board: ${error.message}`);
				  const contin = confirm(`Deleting the existing board failed. Continue anyway?`);
				  if (!contin) {
					  console.log('Import cancelled by user');
					  return null;
				  }
			...
			const createdBoard = await makeAPICall('/boards', 'POST', newBoardData);
			boardId = createdBoard.id;
			console.log(`Created board: ${createdBoard.title} (ID: ${boardId})`);
			
			// Delete all existing labels that where created by default
			console.log("Delete all labels...");
			for (const label of createdBoard.labels) {
				//console.log(label.id, label);
				try {
					await makeApiCall(`/boards/${boardId}/stacks/${label.id}`, 'DELETE');
				} catch {
			...
			//console.log('Existing labels', createdBoard.labels);
			//console.log('New labels', boardData.labels);
			const labelMapping = {};
			if (boardData.labels && boardData.labels.length > 0) {
			  for (const label of boardData.labels) {
				  try {
					let createdLabelId = null;
					// check if label already exists (could happen because of auto generated labels which could not be deleted)
					...
					if (existingLabel) {
						...
						const createdLabel = await makeAPICall(`/boards/${boardId}/labels`, 'POST', labelData);
			...
			if (boardData.stacks && boardData.stacks.length > 0) {
			  // Sort stacks by order to maintain the same arrangement
			  //const sortedStacks = [...boardData.stacks].sort((a, b) => a.order - b.order);
			  ...
			  for (const stack of boardData.stacks.reverse()) {
				...
				const createdStack = await makeAPICall(`/boards/${boardId}/stacks`, 'POST', stackData);
				const stackId = createdStack.id;
				console.log(`Created stack: ${stack.title} (ID: ${stackId})`);
				
				// Create cards in this stack
				if (stack.cards && stack.cards.length > 0) {
				  ...
				  for (const card of sortedCards) {
					...
					const cardData = {
					  ...
					  done: card.done || null,  // Include the done field
					  archived: card.archived || false
					};
					
					const createdCard = await makeAPICall(`/boards/${boardId}/stacks/${stackId}/cards`, 'POST', cardData);
					const cardId = createdCard.id;
					console.log(`Created card: ${card.title} (ID: ${cardId})`);
					//console.log(card, cardData, createdCard);
					...
					if (card.done) {
						...
						const modifiedCard = await makeAPICall(`/boards/${boardId}/stacks/${stackId}/cards/${cardId}`, 'PUT', createdCard);
						//console.log("modifiedCard", modifiedCard);
					}
					
					// Assign labels to the card
					console.log("Asign labels to card...");
					if (card.labels && card.labels.length > 0) {
					  for (const label of card.labels) {
						  const labelId = label.id;
						const newLabelId = labelMapping[labelId];
						if (newLabelId) {
						  await makeAPICall(`/boards/${boardId}/stacks/${stackId}/cards/${cardId}/assignLabel`, 'PUT', {
							labelId: newLabelId
						  });
					...
					if (card.assignedUsers && card.assignedUsers.length > 0) {
					  for (const user of card.assignedUsers) {
						try {
						  await makeAPICall(`/boards/${boardId}/stacks/${stackId}/cards/${cardId}/assignUser`, 'PUT', {
					...
					if (card.attachments && card.attachments.length > 0) {
					  console.log(`Card ${card.title} has ${card.attachments.length} attachments - manual upload may be required`);
					}
					...
					if (card.comments && card.comments.length > 0) {
					  for (const comment of card.comments) {
						try {
						  await makeAPICall(`/cards/${cardId}/comments`, 'POST', {
			...
			// Set board permissions if they exist
			if (boardData.acl && boardData.acl.length > 0) {
			  for (const acl of boardData.acl) {
				try {
				  ...
				  await makeAPICall(`/boards/${boardId}/acl`, 'POST', aclData);
	...
	// frappe-gantt section ----->	
        function createGanttChart() {
            ...
            if (!boardData || !boardData.stacks) {
                showError('No bord data available');
                return;
            }
            ...
            boardData.stacks.forEach((stack, stackIndex) => {
                ...
                if (stack.cards && stack.cards.length > 0) {
                    stack.cards.forEach((card, cardIndex) => {
						...
						//console.log("Filter by label title");
						if (cardFilter.labels.length) {
							labelData = card.labels.filter(label => cardFilter.labels.includes(label.title));
							//console.log("card.labels",card.labels);
							//console.log("labelData",labelData);
							if(labelData.length != cardFilter.labels.length) {  // card has to have ALL labels in filter
						...
						cardId2taskIndex[card.id] = taskIndex;
						taskIndex++;
                    });
                }
            });
            ...
            if (tasks.length === 0) {
				...
				try {
					...
					popup.classList.add("hide");  // make sure the popup is hidden at the beginnung (frappe gantt creates an empty element for the popup, but does not add the class hide at the geginning)
		...
		function createTaskName(card){
			...
			if (card.id in pendingCardUpdates) {
				taskName +=  ` <tspan dy='-2' fill='#9b9b9b' font-size='18px' font-weight='bold'>&#x21BB;</tspan>`;  // &#128472; &#x1F5D8;
			}
			taskName +=  `</tspan>`;
			return taskName;
		}
        ...
        function displayLabels() {
            ...
            if (allLabels.size > 0) {
                ...
                allLabels.forEach(label => {
					...
					// Eventlstener for Filter by label:
					indicator.onclick = function(event){
						// if label was already added to filter
						if (cardFilter.labels.includes(label.title)) {
							// remove label from filter:
							const index = cardFilter.labels.indexOf(label.title);
							if (index > -1) { // only splice array when item is found
							  cardFilter.labels.splice(index, 1); // 2nd parameter means remove one item only
							}
						// if label was not already added to filter
						} else {
							// add label to filter
							cardFilter.labels.push(label.title);  
					...
					if (cardFilter.labels.includes(label.title)) {
						...
						const rgb = indicator.children[0].style.backgroundColor.replace("rgb(","").replace(")","").split(", ");
	...
	// markdown section ---->
		...
		function tuneHtmlPopup(html, markdown, parent_id='') {
			...
			//const md_rows = Math.max(md_lines.length + 0,3);
			...
			html = `
					<div class="html_box" id="${parent_id}"><span>
						${html}</span>
						<span class="html-md-icon-toggle" id="toggleBtn">${edit_symbol}</span>
					</div>
					<div class="md_box hide">
						<textarea class="md_textarea">${markdown}</textarea>
						<span class="html-md-icon-toggle" id="toggleBtn2">${confirm_symbol}</span>
					</div>
					`;
					// pen &#9998;
					// check &#x2611;
			...
			if( html.replaceAll('\n','').replaceAll(' ','').length ) {
				html += '<hr style="border: 1px solid #e9e9e9; margin-top: 12px; margin-bottom: 6px;">';
			}
			// add horizontal line at the beginning
			html = '<hr style="border: 1px solid #e9e9e9; margin-top: 6px; margin-bottom: 12px;">' + html;
	...
	// regular updates ---->
		...
		async function checkRemoteBoardUpdates(enforce=false) {
			...
			try {
				// before checking for updates, send pending card changes...
				sendPendingCardData();
	...
	// handel changes by user ---->
		...
		function onPopupClose() {
			console.log('Popup closed...', checkbox_changed);
			if (checkbox_changed) {
		...
		function autoResizeHight(el) {
			el.style.height = 'auto'; // Reset
			el.style.height = el.scrollHeight + 4 + 'px'; // Resize to fit content (+2 to be sure)
		}
		function setUpHtmlMdToggle() {
			...
			toggleBtn.addEventListener('click', () => {
			  ...
			  autoResizeHight(textarea);
			});
			htmlBox.addEventListener('click', (event) => {
				//console.log("event.target.type", event.target.type);  
				if (event.target.type != "checkbox") {  // no action here if user clicked a checkbox
					// const pos = document.caretPositionFromPoint(event.clientX, event.clientY);
					// if (pos) {
						// const {offsetNode, offset} = pos;
						// console.log('Node:', offsetNode);
						// console.log('Character offset in node:', offset);
					// }
			...
			// make sure the md textarea is expanded to fit the text without scrowling
			
			textarea.addEventListener('input', () => autoResizeHight(textarea));
			//const timeout = setTimeout(() => {autoResizeHight(textarea);}, 1000);  // execute at beginning
			
			closeBtn.addEventListener('click', () => onPopupCloseClicked());
		}
